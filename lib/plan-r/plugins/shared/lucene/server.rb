...
module PlanR
  module Plugins
=begin rdoc
Shared among all Lucene-based plugins
=end
    module Lucene


=begin rdoc
Lucene Server plugin. Ensures lucene daemon gets started and stopped when
main application is started/stopped.
=end
      class Server
        # OBSOLETE: there is no reason for this to be a plugin
        #extend TG::Plugin
        #name 'Lucene Server'
...
=begin rdoc
Returns a raw DRb object representing a connection to the Lucene Daemon.
This is mainly used for development and debugging.
Note: This method does not increase the usage count of the daemon.
=end
        def self.raw_connection
          conn = PlanR::Application::JRuby.connect 10
          conn.dec_usage
          conn
        end
...
=begin rdoc
Yield a Lucene DRb connection to block.
This connects before invoking the block and disconnects after it returns. The
return value from the block is returned to the caller.
=end
        def self.with_connection(num_retries=10, &block)
          rv = nil
          return rv if (! PlanR::Application::JRuby.running?) 
          conn = PlanR::Application::JRuby.connect num_retries
          rv = yield conn if (conn && block_given?)
          conn.dec_usage
          rv
        end

        def with_connection(num_retries=10, &block)
          self.class.with_connection num_retries, &block
        end

=begin rdoc
Invoke method 'name' on Lucene DRb service.
The arguments array is usually a Hash of arguments.
=end
        def self.invoke(name, *args)
          ...
          begin
            ...
            rv = conn.send(name.to_sym, *args) if conn
          rescue Exception => e
            $stderr.puts "Error in Lucene::Server.send: '#{e.message}'"
            $stderr.puts e.backtrace[0,4].join("\n")
          ensure
            conn.dec_usage if conn
          end
          rv
        end

        def invoke(name, *args)
          self.class.invoke(name, *args)
        end
